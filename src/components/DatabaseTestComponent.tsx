...
export const DatabaseTestComponent: React.FC = () => {
  ...
  const runDatabaseTest = async () => {
    ...
    try {
      ...
      if (dataLogs.length === 0) {
        ...
        results.push(
          `✅ Migration completed: ${migratedLogs.length} data logs migrated`
        );
      }
      ...
      results.push(
        `✅ Found ${tags.length} unique tags: ${tags.slice(0, 5).join(", ")}${
          tags.length > 5 ? "..." : ""
        }`
      );
  ...
  return (
    <div className="p-4 bg-gray-800 rounded-lg border border-gray-700 max-w-md">
      <h3 className="text-lg font-bold text-white mb-3">Database Status</h3>

      <button
        onClick={runDatabaseTest}
        disabled={isRunning}
        className="mb-3 px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-500 disabled:opacity-50 text-sm"
      >
