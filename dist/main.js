...
function ensureDatabase() {
    if (!database) {
        try {
            ...
            console.error("Failed to initialize database in ensureDatabase():", error);
            throw error;
        }
    }
    return database;
...
function setupDatabaseIPC() {
    ...
    ipcMain.handle("database:createNeuronCluster", async (_, input) => {
        try {
            const db = ensureDatabase();
            return db.createNeuronCluster(input);
        }
        catch (error) {
            console.error("Error occurred in handler for 'database:createNeuronCluster':", error);
            throw error;
        }
    });
    ipcMain.handle("database:getNeuronClusterById", async (_, id) => {
        try {
            const db = ensureDatabase();
            return db.getNeuronClusterById(id);
        }
        catch (error) {
            console.error("Error occurred in handler for 'database:getNeuronClusterById':", error);
            throw error;
        }
    });
    ipcMain.handle("database:getAllNeuronClusters", async () => {
        try {
            ...
            console.error("Error occurred in handler for 'database:getAllNeuronClusters':", error);
            throw error;
        }
    });
    ipcMain.handle("database:updateNeuronCluster", async (_, id, updates) => {
        try {
            const db = ensureDatabase();
            return db.updateNeuronCluster(id, updates);
        }
        catch (error) {
            console.error("Error occurred in handler for 'database:updateNeuronCluster':", error);
            throw error;
        }
    });
    ipcMain.handle("database:deleteNeuronCluster", async (_, id) => {
        try {
            const db = ensureDatabase();
            return db.deleteNeuronCluster(id);
        }
        catch (error) {
            console.error("Error occurred in handler for 'database:deleteNeuronCluster':", error);
            throw error;
        }
    });
    // Data Log operations
    ipcMain.handle("database:createDataLog", async (_, input) => {
        try {
            const db = ensureDatabase();
            return db.createDataLog(input);
        }
        catch (error) {
            console.error("Error occurred in handler for 'database:createDataLog':", error);
            throw error;
        }
    });
    ipcMain.handle("database:getDataLogById", async (_, id) => {
        try {
            const db = ensureDatabase();
            return db.getDataLogById(id);
        }
        catch (error) {
            console.error("Error occurred in handler for 'database:getDataLogById':", error);
            throw error;
        }
    });
    ipcMain.handle("database:getAllDataLogs", async () => {
        try {
            ...
            console.error("Error occurred in handler for 'database:getAllDataLogs':", error);
            throw error;
        }
    });
    ipcMain.handle("database:getDataLogsByCluster", async (_, clusterId) => {
        try {
            const db = ensureDatabase();
            return db.getDataLogsByCluster(clusterId);
        }
        catch (error) {
            console.error("Error occurred in handler for 'database:getDataLogsByCluster':", error);
            throw error;
        }
    });
    ipcMain.handle("database:updateDataLog", async (_, id, updates) => {
        const db = ensureDatabase();
        return db.updateDataLog(id, updates);
    });
    ipcMain.handle("database:deleteDataLog", async (_, id) => {
        const db = ensureDatabase();
        return db.deleteDataLog(id);
    });
    // Memory Node operations
    ipcMain.handle("database:createMemoryNode", async (_, input) => {
        const db = ensureDatabase();
        return db.createMemoryNode(input);
    });
    ipcMain.handle("database:getMemoryNodeById", async (_, id) => {
        const db = ensureDatabase();
    ...
    ipcMain.handle("database:getAllMemoryNodes", async () => {
        try {
            ...
            console.error("Error occurred in handler for 'database:getAllMemoryNodes':", error);
            throw error;
        }
    });
    ipcMain.handle("database:getMemoryNodesByDataLogId", async (_, dataLogId) => {
        try {
            const db = ensureDatabase();
            return db.getMemoryNodesByDataLogId(dataLogId);
        }
        catch (error) {
            console.error("Error occurred in handler for 'database:getMemoryNodesByDataLogId':", error);
            throw error;
        }
    });
    ipcMain.handle("database:updateMemoryNode", async (_, id, updates) => {
        const db = ensureDatabase();
        return db.updateMemoryNode(id, updates);
    });
    ipcMain.handle("database:deleteMemoryNode", async (_, id) => {
        const db = ensureDatabase();
        return db.deleteMemoryNode(id);
    });
    // Connection operations
    ipcMain.handle("database:createConnection", async (_, fromNodeId, toNodeId, sharedTags, glitchOffset) => {
        const db = ensureDatabase();
        return db.createConnection(fromNodeId, toNodeId, sharedTags, glitchOffset);
    });
    ipcMain.handle("database:getAllConnections", async () => {
        const db = ensureDatabase();
        return db.getAllConnections();
    });
    ipcMain.handle("database:deleteConnection", async (_, id) => {
        const db = ensureDatabase();
        return db.deleteConnection(id);
    });
    ipcMain.handle("database:regenerateConnectionsForNode", async (_, nodeId) => {
        const db = ensureDatabase();
        return db.regenerateConnectionsForNode(nodeId);
    });
    // Search and utility operations
    ipcMain.handle("database:searchDataLogs", async (_, query) => {
        const db = ensureDatabase();
    ...
    ipcMain.handle("database:getAllTags", async () => {
        try {
            ...
            console.error("Error occurred in handler for 'database:getAllTags':", error);
            throw error;
        }
    });
    ...
    // File operations
    ipcMain.handle("file:saveImage", async (_, imageData, filename) => {
        try {
            ...
            if (originalExt === ".heic" || originalExt === ".heif") {
                ...
                try {
                    ...
                    console.log(`Successfully converted ${filename} to ${finalFilename}`);
                }
    ...
    ipcMain.handle("file:getImagePath", async (_, relativePath) => {
        try {
            const userDataPath = app.getPath("userData");
            const fullPath = path.join(userDataPath, relativePath.replace("./assets/", ""));
