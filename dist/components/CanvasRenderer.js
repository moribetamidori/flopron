...
export const CanvasRenderer = ({ nodes, connections, hoveredNode, selectedNode, sidebarCollapsed, previewMode, imageCache, time, rotationX, rotationY, zoom, onDotHover, onDotLeave, rotateX, rotateY, project3D, }) => {
    ...
    useEffect(() => {
        ...
        connections.forEach((connection) => {
            ...
            // Project to 2D
            const projectedFrom = project3D(rotatedFrom.x, rotatedFrom.y, rotatedFrom.z, sidebarCollapsed);
            const projectedTo = project3D(rotatedTo.x, rotatedTo.y, rotatedTo.z, sidebarCollapsed);
            ...
            const getConnectionColor = (tagCount) => {
                if (tagCount === 0)
                    return "#00ffff"; // Cyan for no shared tags
            ...
            const getConnectionStyle = (tagCount) => {
                ...
                const alpha = Math.min(baseAlpha + (tagCount - 1) * alphaIncrement, maxAlpha);
                ...
                for (let i = 1; i <= maxDots; i++) {
                    ...
                    if (onDotHover) {
                        ...
                        if (mouseX > 0 && mouseY > 0) {
                            const distanceToMouse = Math.sqrt(Math.pow(mouseX - dotX, 2) + Math.pow(mouseY - dotY, 2));
        ...
        const groundGradient = ctx.createLinearGradient(0, canvas.height, 0, canvas.height * 0.3);
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "w-full h-full cursor-pointer", style: { touchAction: "none" } }));
};
