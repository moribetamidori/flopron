...
export const useCanvasInteraction = ({ nodes, previewMode, rotationX, rotationY, sidebarCollapsed, rotateX, rotateY, project3D, updateRotation, updateZoom, onNodeClick, onNodeHover, playNodeSound, }) => {
    ...
    const handleMouseMove = (e) => {
        ...
        const hovered = projectedNodes.find((node) => {
            if (previewMode && node.dataLog) {
                ...
                return (x >= node.projectedX - halfSize &&
                ...
                const distance = Math.sqrt((x - node.projectedX) ** 2 + (y - node.projectedY) ** 2);
                return distance < 30;
            }
        });
        if (hovered) {
    ...
    const handleTouchStart = (e) => {
        if (e.touches.length === 2) {
            ...
            const distance = Math.sqrt(Math.pow(touch1.clientX - touch2.clientX, 2) +
