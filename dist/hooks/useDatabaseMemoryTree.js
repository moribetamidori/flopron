...
export const useDatabaseMemoryTree = () => {
    ...
    const convertToUINode = useCallback((dbNode) => {
        return {
            id: dbNode.id,
            x: dbNode.x,
            y: dbNode.y,
    ...
    const convertToUIConnection = useCallback((dbConnection) => {
        return {
    ...
    const addNode = useCallback(async (dataLog) => {
        try {
            ...
            for (const existingNode of existingNodes) {
                if (existingNode.id === dbNode.id ||
                    !existingNode.dataLog?.tags ||
                    !dataLog.tags)
                    continue;
                // Find shared tags
                const sharedTags = dataLog.tags.filter((tag) => existingNode.dataLog.tags.includes(tag));
                if (sharedTags.length > 0) {
                    // Create connection in database
                    const dbConnection = await databaseService.createConnection(dbNode.id, existingNode.id, sharedTags, Math.random() * 10);
    ...
    const updateNode = useCallback(async (nodeId, updates) => {
        try {
            ...
            if (updatedDbNode) {
                ...
                // Update UI state
                setNodes((prevNodes) => prevNodes.map((node) => (node.id === nodeId ? updatedUINode : node)));
            }
        }
        catch (err) {
    ...
    const searchNodes = useCallback(async (query) => {
        try {
            ...
            for (const dataLog of searchResults) {
                const existingNode = nodes.find((node) => node.dataLog?.id === dataLog.id);
                if (existingNode) {
