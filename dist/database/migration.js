...
export class DataMigration {
    ...
    async migrateExistingData() {
        ...
        try {
            ...
            if (existingDataLogs.length > 0) {
                console.log(`Found ${existingDataLogs.length} existing data logs in database. Skipping migration.`);
                return;
            }
            ...
            for (const dataLog of dataLogs) {
                ...
                const memoryNodeInput = this.databaseService.createMemoryNodeFromDataLog(createdDataLog);
            ...
            for (let j = i + 1; j < memoryNodes.length; j++) {
                ...
                if (sharedTags.length > 0) {
                    ...
                    if (!connectionSet.has(connectionKey)) {
                        ...
                        await this.databaseService.createConnection(nodeA.id, nodeB.id, sharedTags, Math.random() * 10 // glitch offset
                        );
                        console.log(`Created connection between ${nodeA.id} and ${nodeB.id} with shared tags: ${sharedTags.join(", ")}`);
                    }
                }
            }
        }
    ...
    async checkDataIntegrity() {
        ...
        - Connections: ${connections.length}`);
            ...
            for (const dataLog of dataLogs) {
                const correspondingNode = memoryNodes.find((node) => node.data_log_id === dataLog.id);
                if (!correspondingNode) {
                    console.error(`Data log ${dataLog.id} has no corresponding memory node`);
                    return false;
                }
            }
            // Check that each connection references valid nodes
            for (const connection of connections) {
                const fromNodeExists = memoryNodes.some((node) => node.id === connection.from_node_id);
                const toNodeExists = memoryNodes.some((node) => node.id === connection.to_node_id);
