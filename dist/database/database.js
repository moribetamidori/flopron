...
export class PKMDatabase {
    ...
    createDataLog(input) {
        ...
        VALUES (?, ?, ?, ?)
      `);
            insertDataLog.run(input.id, input.title, input.timestamp.toISOString(), input.content);
            // Handle tags
    ...
    updateDataLog(id, updates) {
        const transaction = this.db.transaction(() => {
          ...
          WHERE id = ?
        `);
                updateDataLog.run(updates.title, updates.content, updates.timestamp?.toISOString(), id);
            }
